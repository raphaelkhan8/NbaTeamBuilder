...
const Team = (props) => {
    ...
    const [teamInfo, setTeamInfo] = useState(initialTeam)
    const [showTeam, toggleShowTeam] = useState(false)
    
    useEffect(() => {
        setTeamInfo(props.team)
    })

    const { players, teamWins, averageAge, pointsPG, assistsPG, blocksPG, stealsPG, turnoversPG, personalFoulsPG,
        offRPG, defRPG, offEfficiency, defEfficiency, teamMinutesLeft, twoPtMade, twoPtAttempt, 
        threePtMade, threePtAttempt, totalShootingPercentage, saveTeam, clearTeam, releasePlayer } = teamInfo
